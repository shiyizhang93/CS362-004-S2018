...
//It is to test the smithy card effect
...
int main (int argc, char** argv) {
    ...
    int testVal;
    int tempFlag = 0;
    int numNewCardHand = 3;
    int numCardPlay = 1;
    int playerNum;

    initializeGame(numPlayers, k, seed, &startState);
    endState = startState;
    printf("Test suite for card smithy effect\n");
    for (playerNum = 0; playerNum < numPlayers; playerNum++) {
	    cardEffect(smithy, choice1, choice2, choice3, &endState, handFlag, &bonus);
