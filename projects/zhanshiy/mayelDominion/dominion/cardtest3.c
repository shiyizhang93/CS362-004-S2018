...
//It is to test the council room card effect
...
int main (int argc, char** argv) {
    ...
    int testVal;
    int tempFlag = 0;
    int numNewCardHand = 4;
    int numCardPlay = 1;
    int playerNum;
    int bonusBuyNum = 1;
    int numNewCardHand2 = 1;

    initializeGame(numPlayers, k, seed, &startState);
    endState = startState;
    printf("Test suite for card council room effect\n");
    for (playerNum = 0; playerNum < numPlayers; playerNum++) {
	    cardEffect(council_room, choice1, choice2, choice3, &endState, handFlag, &bonus);
      ...
      expectedVal = playerNum == 1 ?
	      startState.deckCount[playerNum] - numNewCardHand - numNewCardHand2 : startState.deckCount[playerNum] - numNewCardHand;
	    testVal = endState.deckCount[playerNum];
	    showTestResult(expectedVal, testVal, "Player deck number"); 
      ...
      expectedVal = startState.numBuys + bonusBuyNum;
	    testVal = endState.numBuys;
	    showTestResult(expectedVal, testVal, "Player number of buys");

    	expectedVal = playerNum == 1 ? 
    	    startState.deckCount[1-playerNum] - numNewCardHand - numNewCardHand2 : startState.deckCount[1-playerNum] - numNewCardHand2;
